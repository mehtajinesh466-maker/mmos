...
function tryParameters(nameFirst, updateNameMap, fallbackStatusFromPackage, centreFromPackage) {
  ...
  studentData.forEach((row, idx) => {
    ...
    let status = String(row['Status'] || 'Active').trim().toLowerCase() === 'active' ? 'active' : 'inactive';
  ...
  packageData.forEach(row => {
    ...
    if (studentId) {
      ...
      students.push({
        ...
        refId: ref,
        status,
        centre: centreFromPackage ? (row['Assigned center'] || 'Bay Avenue') : 'Bay Avenue',
        lastAttended: null
      });
    }
  ...
  const totalUnbilled = totalActiveUnbilled + totalInactiveUnbilled;
  console.log(`nameFirst=${nameFirst}, updateNameMap=${updateNameMap}, fallbackStatus=${fallbackStatusFromPackage}, centreFromPkg=${centreFromPackage}:`);
  console.log(`  Total Students: ${students.length} (Active: ${activeCount}, Inactive: ${inactiveCount})`);
  console.log(`  Total Unbilled: AED ${totalUnbilled.toLocaleString()} (Active: AED ${totalActiveUnbilled.toLocaleString()}, Inactive: AED ${totalInactiveUnbilled.toLocaleString()})`);
}

// Let's run a sweep
for (let nameFirst of [true, false]) {
  for (let updateNameMap of [true, false]) {
    for (let fallbackStatus of [true, false]) {
      for (let centreFromPkg of [true, false]) {
        tryParameters(nameFirst, updateNameMap, fallbackStatus, centreFromPkg);
      }
    }
  }
}
