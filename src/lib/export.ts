  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < cols.length; j++) {
      ...
      if (select) {
        ...
        // Clone the cell to clean up decorative/action elements without modifying the active UI
        const clone = htmlElement.cloneNode(true) as HTMLElement;
        
        // Remove action buttons, icons, and decorative badges so button text like "✎" or "✗" is removed while preserving class & student text
        clone.querySelectorAll("img, button, svg").forEach(el => el.remove());
        clone.querySelectorAll("div, span").forEach(el => {
          const classList = el.className || "";
          if (
            classList.includes("rounded-full") &&
            (classList.includes("w-6") || classList.includes("w-8") || classList.includes("h-6") || classList.includes("h-8"))
          ) {
            el.remove();
          }
        });
